...
class GameResultServiceTests {
    ...
    fun test_deleteGameResult_removesCorrectEntry() {
        ...
        service.deleteGameResult(1) // löscht player1

        val results = service.getGameResults()
        assertEquals(1, results.size)
        assertEquals(gameResult2, results[0])
    }

    @Test
    fun test_deleteGameResult_notFound_changesNothing() {
        val gameResult = GameResult(0, "player1", 17, 15.3)
        service.addGameResult(gameResult)

        service.deleteGameResult(99) // ID existiert nicht
